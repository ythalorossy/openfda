...
- `src/core/http.ts`'s `fetchPage()` further classifies an HTTP response into `hit` / `miss` (openFDA's zero-match 404) / `bad_request` (openFDA rejected the request outright, e.g. a `count`/`sort` field its index no longer accepts) / `error` (a genuine upstream failure) — a zero-match search must never be reported as `isError: true`, and a `bad_request` must never be retried or reported as an outage or an empty dataset
...
| `src/core/executor.ts` | `execute()` — the one request pipeline every tool's handler runs; `COUNT_BUCKET_DEFAULT` (100) is the bucket ceiling used when `count` is set and `limit` is absent |
| `src/core/paging.ts` | `SKIP_MAX` (25000) — openFDA's own paging offset ceiling, factored out of `executor.ts` because `shape/envelope.ts` needs it too |
...
| `src/core/http.ts` | `fetchPage()` — one openFDA page request, classified into hit / miss / bad_request / error |
...
| `src/catalog/drug-<endpoint>.json` / `.coverage.json` / `.countable.json` | The committed, offline FDA field reference, coverage data and countability measurement every descriptor is checked against (`tests/catalog-conformance.test.ts`); regenerated by `npm run fields:sync` / `npm run fields:coverage` / `npm run fields:countable` |
...
`src/core/registry.ts`'s `toToolDefinition()` turns that object into the tool's Zod `inputSchema`, its description string, and a handler that is just `execute(descriptor, input)`. `validateDescriptor()` runs first and refuses to register a descriptor with structural problems (duplicate names, an empty strategy, a schema key that collides with a built-in parameter, `limits.default` above `limits.max`, and more). `countFields` is additionally checked, offline, against `src/catalog/drug-<endpoint>.countable.json` — being published and populated does not imply openFDA's index will actually aggregate a path.
...
2. Reject `skip` above openFDA's `SKIP_MAX` (25000) ceiling and `count` values not declared in the descriptor's `countFields`; resolve `detail` to a `Projection`.
...
5. Fetch a page via `fetchPage()` (`core/http.ts`), classified as `hit` / `miss` (a zero-match search — data, not an error) / `bad_request` (openFDA rejected the request outright — reported to the caller, never retried or read as an outage) / `error` (a genuine upstream failure).
6. On a hit: project records through the chosen `Projection`, wrap them in the shared envelope (`matched_via`, `total`, `returned`, `limit`, `dropped_for_budget`, `next_skip`, `results`), and trim to the 60,000-character response budget, reporting how many rows were dropped. `next_skip` is the sound offset to resume paging from — `null` when exhausted or when the next offset would exceed `SKIP_MAX` — and callers should advance by it, not by `skip + limit`, since the budget can drop trailing rows. `limit` caps rows for a record search but buckets for `count`; an absent `limit` under `count` defaults to `COUNT_BUCKET_DEFAULT` (100) rather than the record default. On `count`: decode ranked terms via `decodeTerm()` against the descriptor's `codeMaps`, then trim buckets through that same budget — the aggregated payload reports `dropped_for_budget` but no `next_skip`, which an aggregation has no semantics for.

## Available Tools

Every response envelope carries `matched_via`, `total`, `returned`, `limit`, `dropped_for_budget` (rows dropped to fit the 60,000-character budget, 0 when none, never omitted), `next_skip` (the offset to resume from — `null` when exhausted or when it would exceed `SKIP_MAX`) and `results`. **Page by `next_skip`, not `skip + limit`.** Under `count`, `limit` caps buckets and defaults to 100 when omitted, but an explicit `limit` is still capped by the tool's record `max` below — so on every tool except `drug-drugsfda` (max 100) a caller can receive more buckets by omitting `limit` than it can explicitly request. Record maxes: `drug-label` 25, `drug-event` 50, `drug-drugsfda` 100, `drug-ndc` 50, `drug-enforcement` 50, `drug-orangebook` 50, `drug-shortages` 50. An aggregated response is trimmed to that same budget and reports `dropped_for_budget` for the buckets it dropped, `returned` counting those kept; it carries no `next_skip`, an aggregation having no result total and no skip semantics.
