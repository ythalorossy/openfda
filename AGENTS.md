...
| `src/datasets/drug/label.ts`, `event.ts`, `drugsfda.ts`, `ndc.ts`, `enforcement.ts` | One `EndpointDescriptor` each: `drug-label`, `drug-event`, `drug-drugsfda`, `drug-ndc`, `drug-enforcement` |
