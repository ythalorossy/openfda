...
Every tool takes `field` + `value`, `limit`/`skip`, and `detail`; `sort` and `count` are present only when the descriptor declares them. The response envelope always carries `matched_via` (which path actually matched), `total` (the upstream match count, not the number of rows returned), `limit`, `dropped_for_budget` (rows dropped to stay inside the 60,000-character budget — 0 when none were, never omitted), and `next_skip` (the offset to resume from; `null` when the result set is exhausted or the next offset would exceed `SKIP_MAX`). **Advance paging by `next_skip`, not `skip + limit`** — the response budget can drop trailing rows, so `skip + limit` silently steps over exactly the rows that were dropped. A zero-match search returns a plain no-results message, not an error, and every response is capped at 60,000 characters.
