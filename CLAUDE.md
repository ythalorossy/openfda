...
- **`tests/`**: Vitest test suite (335 tests across 36 files: ApiHandler, catalog-conformance, catalog-coverage, catalog-shape, core/budget, core/codes, core/descriptor, core/envelope, core/escape, core/executor, core/http, core/project, core/query, core/registry, core/strategy, datasets/drug-drugsfda, datasets/drug-event, datasets/drug-label, datasets/faers, docs-currency, drift-guard, env, faers-codes, format, label-fields, ndc, ndc-formats, no-raw-query, no-url-in-output, OpenFDABuilder, redact, registration, schema-budget, ToolManager, ToolManager.keyguard, version)
...
- **`src/utils/ndc.ts`**: `normalizeNDC()` — the single validator/normalizer for `drug-label`'s `ndc` field. Accepts 4-4, 5-3 and 5-4 product NDCs and their package forms; rejects ambiguous undashed 8- and 10-digit input.
...
- **`src/utils/ndc-formats.ts`**: `NDC_FORMATS` / `invalidNdcMessage()` — the accepted-NDC-format block rendered in `drug-label`'s NDC error messages.
